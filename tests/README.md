...
`pnpm test` starts and stops an isolated Vite server and Chromium. It renders the real server-sync dialog and shared controls, with Tauri IPC as the external boundary. Deferred responses exercise preview, deployment, config-policy saves, and worker-automation saves. Unexpected IPC commands fail the tests. No game server or saved credentials are used.
